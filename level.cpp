...
void Level::loadMap(std::string mapName, Graphics &graphics) {
				...
				while (pData) {
					...
					if (pTile != NULL) {
						...
						while (pTile) {
							...
							//Build the acutal tile and add it to the level's list
	...
	if (pObjectGroup != NULL) {
		while (pObjectGroup) {
				...
				if (pObject != NULL) {
					while (pObject) {
						...
						if (pPolyline != NULL) {
							...
							//split the polyline coordinates into pairs
							std::stringstream ss;
							ss << pointString;
			...
			//parses out all the starting spawn points from the *.tmx file
			...
			//parse out the doors from the *.tmx file
